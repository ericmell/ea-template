...
<#macro digui_systempara systemparaNodes  flag >
	<#list systemparaNodes as systempara>
	    <tr>
	    	<td >
	    		${flag}<b><#if flag=="">参数类别<#else>-[key]</#if></b><input  value="${systempara.key?if_exists}" 
			      style="WIDTH: 180px"  onchange="javascript:action_systempara('update.do','column=key&id=${systempara.id?if_exists}&columnValue='+escape(encodeURIComponent(this.value)))" /> 
